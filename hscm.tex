...
g_\xi\left( t \right) = \mathop \sum \limits_{l = 1}^L {{\rm{\Xi }}_l}\left( t \right){e^{j{{\rm{\Psi }}_l}\left( t \right)}}{e^{ - j{{\vec k}_{\xi l}(t)}\cdot{{\vec r}_{\xi l}}(t)}}{e^{j{\phi _{l 0}}}},
\end{equation}
where $\xi\!\in\!\left\{1,2\right\}$ is the index of MS antenna, ${\vec k}_{\xi l}(t)$ and ${\vec r}_{\xi l}(t)$ are the wave vector of the $l$th path impinging the MS and the relative position vector from the corresponding scatterer to the MS antennas, respectively. $\big\|{{\vec k}_{\xi l}(t)}\big\|\!=\!{2\pi}/{\lambda}$ is the angular wavenumber.
${\vec r}_{\xi l}(t+\tau)={\vec r}_{\xi l}(t)+{\vec v}\tau$, ${\vec v}$ is the velocity vector of the MS.
${{\rm{\Xi }}_l}\!\left( t \right)$, ${{\rm{\Psi }}_l}\!\left( t \right)$ are two independent random variables representing the random reflection coefficient and the phase shift of the $l$th path, respectively.
${\phi _{l 0}}$ is  the random initial phase of the $l$th path.
...
g_\xi\left( t \right) = \mathop \sum \limits_{l = 1}^L {A_l}\left( t \right){e^{ - j{{\vec k}_{\xi l}(t)}\cdot{{\vec r}_{\xi l}}(t)}}{e^{j{\phi _{l 0}}}}.
\end{equation}
...
\caption{Geometric scattering model for multi-path propagation.} \label{mpm}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.5\textwidth]{idea0.eps}
\caption{The variation process of local scattering.} \label{idea0}
\end{figure}
The received signal at MS is the sum of multi-path waves impinging from any angle scattered by surrounding local objects.
...
However, with the velocity increasing or scattering intensified, this assumption tends to deviate from reality, making the previous channel models less accurate. As shown in Fig. \ref{idea0}, as the mobile passes through the local scatterers, the signal component from a certain incidence angle, e.g., $\theta_l$, may be blocked by or jump to another object. We call this dynamics of the signal components reflected through these surrounding objects \emph{scatterer switching events}. And the dynamics is a temporal stochastic  process  determined by both the objects' spatial distribution and the MS's moving speed.
...
Without loss of generality, each local scatterer is assumed to be a single reflector, thus signals reflected by different scatterers are uncorrelated.
And we assume  the spatial distribution of surrounding objects to be homogeneous Poisson point process, which is commonly used \cite{spatial}.
...
Given that the MS moves at a constant speed, the spatial Poisson process is mapped to a temporal Poisson process, which means the scatterer switching events follow a uniform Poisson process.
...
Hence, we propose a new channel model with the variation  of reflecting scatterers taken into account as Fig. \ref{idea1}.
...
The dynamics of { path gain process} may be depicted as follows,
...
\begin{figure}[t]
\centering
\includegraphics[width=0.5\textwidth]{idea1.eps}
\caption{Variation process of multi-path waves of each incidence angle.} \label{idea1}
\end{figure}
...
{\mathbb{E}\!\left[ {{A_l}\left( t \right)\!{A_l}^*\!\left( {t \!+\! \tau } \right)} \right]{\mathbf{E}_{\Theta}\!\!\left[e^{j{2\pi {f_m}\!\tau\! \cos \left( {\beta  - {\theta_l}} \right) - j\frac{{2\pi }}{\lambda }d\cos \left( {{\theta_l  }} \right)} }\right]}},
\end{aligned}
\end{equation}
where $f_m=v/{\lambda}$ is the maximum Doppler frequency, and $\theta_l$, $l\!=\!1, 2,\cdots,L$, are i.i.d von Mises distributed AOAs.
$(a)$ is because the wave vectors are approximately parallel within the interval.
