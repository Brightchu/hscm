...
However, with the velocity increasing or scattering intensified, this assumption tends to deviate from reality, making the previous channel models less accurate. As shown in Fig. \ref{idea0}, as the mobile passes through the local scatterers, the signal component from a certain incidence angle, e.g., $\theta_l$, may be blocked or reflected by another object. We call this dynamics of the signal components \emph{scatterer switching events}.
\begin{figure}[tH]
\centering
...
Given that the MS moves at a constant speed, the spatial Poisson process is mapped to a temporal Poisson process, which means the scatterer switching events follow a Poisson process.
